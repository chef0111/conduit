...
/** Polyfill for motion's usePageInView (not in motion@12.42). */
function usePageInView() {
  const [isPageInView, setIsPageInView] = useState(true);

  useEffect(() => {
    const update = () => {
      setIsPageInView(document.visibilityState === 'visible');
    };
...
export function LogosCarousel({
  ...
  return (
    <div
      ...
      className={cn('grid h-10', className)}
      style={{
